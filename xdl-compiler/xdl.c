...
xdl_model* xdl_new()
{
  ...
  c->types = g_slist_append(c->types, xdl_typedef_new(TD_BASE, "boolean",  "boolean",  "int",    "-1",    "xr_value_to_bool", "xr_value_bool_new", NULL));
  ...
  c->types = g_slist_append(c->types, xdl_typedef_new(TD_BLOB, "blob",     "blob",     "xr_blob*",  "NULL",  "xr_value_to_blob", "xr_value_blob_new", "xr_blob_free"));
